...
<fieldset>
	...
	<!--
		...
		숫자로 변경할 수 없는 문자가 포함된다면 예외가 발생된다.
		또한 문자열끼리의 연결도 EL에서는 허용되지 않는다.
	-->
	<h2>+를 숫자가 아닌 문자열 연결에 사용시(에러 발생)</h2>
	<ul>
	...
	<ul>
		...
		<li>\${header.user-agent}: ${header.user-agent} [에러x, 0 출력]</li>
	</ul>
	...
	<!-- 
		...
		해당 문장은 배열이나 컬렉션의 크기만큼 반복하는 확장 for문 형태로 기술된다.
	 -->
	...
	<!-- 
		...
		EL에서는 기본적인 예외를 잡아주므로 에러가 발생되지는 않는다.
	 -->
	<h2>인덱스를 벗어난 경우</h2>
	<ul>
		<li>\${elArray[3]}
	...
	<ul>
		<li>\${requestScope.nop} : ${requestScope.nop} 
			[Scope계열 내장객체, 에러 x, 미출력 -> 속성명은 개발자 마음대로 정할 수 있기 때문]
		</li>
		<!-- 
		...
		<li>\${pageContext.nop} : \${pageContext.nop}
			[컴파일 오류 발생 -> 내장객체명은 정해져 있기 때문]
		</li>
	</ul>
	
	<h2>EL의 내장객체중 사용자가 정의한 속성에 접근하는 내장객체 및 맵 컬렉션인 경우</h2>
	<%
		request.setAttribute("reqProperty", "리퀘스트 영역 저장");
		Map map = new HashMap();
	...
	<ul>
		...
		<li>\${requestScope.REQProperty }:
			${requestScope.REQProperty } [에러x, 미출력]</li>
		<li>\${elMap.mapKey }:${elMap.mapKey } [정상]</li>
		<li>\${elMAP.mapKey }:${elMAP.mapKey } [에러x, 미출력]</li>
	</ul>
	
	<h2>숫자를 0으로 나누면 INFINITE</h2>
	\${10 div 0 } : ${10 div 0 }
	
	<h2>EL변수에 값 할당 시 [톰캣 8.0부터는 정상. 이전 버전은 에러]</h2>
	<c:set var="number" value="100" scope="request"/>
	
	데이터 저장 전: ${number }<br>
	데이터 저장 후: ${number=1 }<br>
